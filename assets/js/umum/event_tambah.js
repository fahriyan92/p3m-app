...
$(document).ready(function () {
  ...
  const get_data = async () => {
    ...
    $(".bodynya").html(data[0]);
    $(".bodynya-penelitianplp").html(data[1]);
    $(".bodynya-pengabdian").html(data[2]);
    datatable = $(
      "#tabelevent,#tabelevent-penelitianplp,#tabelevent-pengabdian"
    ).DataTable(obj);
  };

  $("#tabelevent,#tabelevent-penelitianplp,#tabelevent-pengabdian").on(
    "click",
    ".btn-edit",
    function (e) {
      e.preventDefault();
  ...
  const validasi = () => {
    ...
    if (make_akhir) {
      console.log($("input[name='akhir']").val());
      console.log($("input[name='akhir']").val() === "");
    }
    ...
    if (state.jns === true && state.thp === true && state.mulai === true) {
      if (make_akhir) {
        if ($('input[name="akhir"]').val() === "") {
          error.akhir
            .find("h5")
            .text("Tanggal Akhir Harus Lebih Dari Tanggal Mulai");
          $(".error-akhir>h5").text("Tidak Boleh Kosong");
          $(".error-akhir").show();
          return false;
        } else {
          const mulai = new Date($('input[name="mulai"]').val());
          const akhir = new Date($('input[name="akhir"]').val());
