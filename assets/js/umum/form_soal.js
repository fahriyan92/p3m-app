...
const controller3 = (function (data, ui) {
  ...
  const simpanHandler3 = function (e) {
    ...
    data
      .store({ soalpge, jnsSoalpge, pilihanpge, bobotpge, persen })
      .then((res) => {
        let title = "Berhasil Menambahkan Data!";
        let status = "success";
        console.log(res);
...
const ui5 = (function () {
  const domstring5 = {
    form: {
      jnsSoalplp: "#jnsSoalplp",
      soalplp: "#soalplp",
      pilihanplp: 'input[name="pilihanplp[]"]',
      bobotplp: 'input[name="bobotplp[]"]',
      persenplp: 'input[name="prosentaseplp[]"]',
    },
  };
...
const controller5 = (function (data, ui) {
  const domstring5 = ui.getDomstring();

  const element = {
    jnsSoalplp: "#jnsSoalplp",
    soalplp: "#soalplp",
    pilihanpge: 'input[name="pilihanplp[]"]',
    bobotpge: 'input[name="bobotplp[]"]',
    persenplp: 'input[name="prosentaseplp[]"]',
  };

  const kosongkan5 = function () {
    $(element.jnsSoalplp).val("").trigger("change");
    $(element.soalplp).val("");
    $(element.pilihanplp).val("");
    $(element.bobotplp).val("");
    $(element.persenplp).val("");
  };
  ...
  const validasi = () => {
    ...
    $('input[name="pilihanplp[]"').each((item, wowo) => {
      if ($(wowo).val().trim() === "") {
        $(wowo).parent(".form-group").next().show();
        state.pilihan = false;
      } else {
    ...
    $('input[name="bobotplp[]"]').each((item, wowo) => {
      if ($(wowo).val().trim() === "") {
        $(wowo).parent(".form-group").next(".err-pilihanplp").show();
        state.bobot = false;
      } else {
        $(wowo).parent(".form-group").next(".err-pilihanplp").hide();
        state.bobot = true;
      }
    });
  ...
  const eventHandler5 = function () {
    $("#simpan-soalplp").on("click", simpanHandler5);
  };
  ...
  const simpanHandler5 = function (e) {
    e.preventDefault();
    ...
    const jnsSoalplp = $(domstring5.form.jnsSoalplp).val();
    const soalplp = $(domstring5.form.soalplp).val();
    const pilihanplp = $(domstring5.form.pilihanplp)
      .map((item, i) => $(i)[0].value)
      .get();
    const bobotplp = $(domstring5.form.bobotplp)
      .map((item, i) => $(i)[0].value)
      .get();
    const persenplp = $(domstring5.form.persenplp)
      .map((item, i) => $(i)[0].value)
      .get();
    // console.log(jnsSoalplp);
    // console.log(soalplp);
    // console.log(pilihanplp);
    // console.log(bobotplp);
    // console.log(persenplp);
    // return;
    data
      .store({ soalplp, jnsSoalplp, pilihanplp, bobotplp, persenplp })
      .then((res) => {
        let title = "Berhasil Menambahkan Data!";
        let status = "success";
        console.log(res);
...
$("#addpilihanplp").click(function () {
  ...
  newSelectDiv.after().html(`<div class="row">
                                <div class="col-sm-6">
                                    <div class="form-group">
                                        <label for="pilihanplp${
                                          counter + 1
                                        }">Pilihan Ganda ${counter + 1}</label>
                                        <input type="text" class="form-control" id="pilihanplp${
                                          counter + 1
                                        }" placeholder="Pilihan Ganda" name="pilihanplp[]" value="">
                                    </div>
                                    <div class="err-pilgan${
                                      counter + 1
                                    }" style="display:none;">
                                ...
                                <div class="col-sm-2">
                                    <div class="form-group">
                                        <label for="bobotplp2">Bobot ${
                                          counter + 1
                                        }</label>
                                        <input type="number" class="form-control" id="bobotplp${
                                          counter + 1
                                        }" placeholder="Bobot" name="bobotplp[]" value="">
                                    </div>
                                    <div class="err-bobot${
                                      counter + 1
                                    }" style="display:none;">
                                ...
                                <div class="col-sm-2">
                                  <div class="form-group">
                                      ...
                                      <input type="number" class="form-control" id="prosentaseplp${
                                        counter + 1
                                      }" placeholder="Prosentase(%)" name="prosentaseplp[]" value="">
                                  </div>
                                </div>
                            </div>`);

  newSelectDiv.appendTo(".append-group3");
  counter++;
});

$("#removepilihanplp").click(function () {
  if (counter == 2) {
    alert("Minimal 2 Pilihan Ganda");
    return false;
  }
...
const data6 = (function () {
  ...
  return {
    store6: function (kirim) {
      return insert6(kirim);
    },
  };
})();
...
const controller6 = (function (data, ui) {
  const domstring6 = ui.getDomstring();

  const eventHandler = function () {
    $("#simpan-jenis-kriteriaplp").on("click", simpanHandler6);
  };
  ...
  const simpanHandler6 = function (e) {
    e.preventDefault();
    ...
    const jnsplp = $(domstring6.form.jnsplp).val();
    const bobotjnsplp = $(domstring6.form.bobotjnsplp).val();

    // console.log(jnsplp);
    // console.log(bobotjnsplp);
    // return;

    data.store6({ jnsplp, bobotjnsplp }).then((res) => {
      let title = "Berhasil Menambahkan Data!";
      let status = "success";
      console.log(res);
      // if (res.code != 1) {
