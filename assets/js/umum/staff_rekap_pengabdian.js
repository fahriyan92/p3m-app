(function () {
      ...
      success: (res) => {
        ...
        $(".modal-score").on("click", function (e) {
          ...
          score_modal(id, score, function () {
            ...
            if(status == 0){
              $('.button-action').html('');
              $('.button-action').html('<input type="button" class="btn btn-danger update" data-status="2" value="Tolak"><input type="button" class="btn btn-success update" data-status="1" style="margin-left:8px;" value="Terima">');
            } else if (status == 1){
              $('.button-action').html('');
              $('.button-action').html('<input type="button" class="btn btn-success update" data-status="6" style="margin-left:8px;" value="Lanjut Monev">');
            } else {
              $('.button-action').html('');
              $('.button-action').html('<input type="button" class="btn btn-warning update" data-status="0" value="Kembalikan">');
            }
  ...
  $("#tahun").on("change", function () {
    const judul = [
      ...
      "Proposal pengabdian lolos pendanaan",
      "Proposal pengabdian tidak lolos pendanaan",
  ...
  $("#download-excel").on("click", function(e){
    ...
    let link = BASE_URL + "export_excel/export_laporan_akhir?id_event=PENGABDIAN_DOSEN&tahun="+tahun;
