$(document).ready(function () {
    ...
    $(`.btn-dosen`).click(function (e) {
      ...
      $.ajax({
        url: `${BASE_URL}C_Pemonev1/get_anggota_dosen`,
        method: `POST`,
    ...
    $(`.btn-mhs`).click(function (e) {
      ...
      $.ajax({
        url: `${BASE_URL}C_Pemonev1/get_anggota_mhs`,
        method: `POST`,
        data: { id_proposal: id_proposal },
        dataType: `json`,
        success: function (res) {
        ...
        success: function (res) {
          if(status == "lanjut_review" || status == "tolak_administrasi" ){
            if (res.code == 1) {
              ...
              setTimeout(function () {
                window.location.replace(`${BASE_URL}C_Pemonev1/tambahPemonev/${id_proposal}`);
              }, 1000);
            } else {
            ...
            if (res.code == 1) {
              swal_alert(`${res.pesan} ${status}`, "success");
              setTimeout(function () {
                window.location.replace(`${BASE_URL}C_Pemonev1`);
              }, 1000);
            } else {
              swal_alert(`${res.pesan} ${status}`, "error");
              setTimeout(function () {
