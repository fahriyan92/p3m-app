const get_data_all = () =>
  new Promise((resolve, reject) => {
    $.ajax({
      url: `${BASE_URL}C_master_data/get_list_master_reviewer`,
      type: "get",
      dataType: "json",
...
const get_by_id = (id) =>
  new Promise((resolve, reject) => {
    $.ajax({
      url: `${BASE_URL}C_master_data/get_list_reviewer_by_id`,
      type: "POST",
      data: id,
      dataType: "json",
      success: (res) => resolve(res),
...
$(document).ready(function () {
  ...
  $(document).on("click", ".btn_delete", function (e) {
    ...
    function deleting(isConfirm) {
      if (isConfirm.value) {
        $.ajax({
          url: `${BASE_URL}C_master_data/deleteReviewer`,
          type: "post",
          data: { id: id },
          dataType: "json",
          success: function (res) {
  ...
  $("#edit-btn").on("click", function (e) {
    ...
    if ($("select[name='reviewer2']").val() === "") {
      ...
      $("#overlay").fadeIn(100);
      $.ajax({
        url: `${BASE_URL}C_master_data/updateReviewer`,
        type: "post",
        data: {
          id_reviewer: $("#id_reviewer").val(),
          reviewer: $("select[name='reviewer2']").val(),
          event: eventIds,
        },
        dataType: "json",
        success: function (res) {
          ...
          swal_alert(title, res.pesan, status);
          window.location.replace(`${BASE_URL}C_master_data/reviewer`);
          $("#overlay").fadeOut(400);
        },
      });
    }
  ...
  $("#simpan-btn").on("click", function (e) {
    ...
    if ($("select[name='reviewer']").val() === "") {
      ...
      $.ajax({
        url: `${BASE_URL}C_master_data/store_MasterReviewer`,
        type: "post",
        data: {
