...
$(element.jnsproposal).on("change", function () {
  var id = $(this).val();
  $.ajax({
    url: `${BASE_URL}C_penelitian_plp_pnbp/cari_biaya`,
    method: "POST",
    data: {
      id: id
    },
...
function cek_anggota2() {
  $('input[name="pakegak"]').on("change", () => {
    if ($('input[name="pakegak"]').is(":checked")) {
      $('#checklist_anggota3').removeClass('d-none');
      console.log('test3');
      make_anggota2 = true;
    } else {
      make_anggota2 = false;
...
function cek_anggota3() {
  $('input[name="pakegak3"]').on("change", () => {
    if ($('input[name="pakegak3"]').is(":checked")) {
      $('#checklist_anggota4').removeClass('d-none');
      console.log('test2');
      make_anggota3 = true;
    } else {
      make_anggota3 = false;
...
function cek_anggota4() {
  $('input[name="pakegak4"]').on("change", () => {
    if ($('input[name="pakega4"]').is(":checked")) {
      make_anggota4 = true;
    } else {
      make_anggota4 = false;
    }
...
$("#b_test2").click(function () {
  ...
  const tema = $('input[name="tema_penelitian"]:checked').val();
  ...
  $.ajax({
    ...
    url: `${
      $(this).val() === "simpan"
        ? `${BASE_URL}C_penelitian_plp_pnbp/store_pengajuan`
        : `${BASE_URL}C_penelitian_plp_pnbp/store_pengajuan`
    }`,
    data: form_data,
    contentType: false,
...
$("#b_test").click(function () {
  if ($("#proposal").val() == '' || $("#rab").val() == '') {
    swal_alert(
      "Gagal",
      "File Proposal & File RAB Tidak Boleh Kosong!",
      "error"
    );
    return;
  } else {
  ...
  $.ajax({
    type: "POST",
    dataType: "json",
    url: `${BASE_URL}C_penelitian_plp_pnbp/store_pengajuan_kedua`,
    data: form_data,
    contentType: false,
    cache: false,
    processData: false,
