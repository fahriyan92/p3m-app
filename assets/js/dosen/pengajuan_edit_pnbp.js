...
$(element.jnsproposal).on("change", function () {
  ...
  $.ajax({
    ...
    success: function (val) {
      ...
      //tambahan niko bug ketika check malah ilang 
      const kelompok = data[0].kelompok_luaran.split(',');
      $(`input[name="luaran[]"]:checked`).prop('checked',false);
      kelompok.forEach(item => {
        $(`input[name="luaran[]"][value="${item}"]`).prop('checked',true);
      });
