...
function checkextensionCV3() {
  const file = document.querySelector("#cvAnggota3");
  if (/\.(pdf)$/i.test(file.files[0].name) === false) {
    file.value = null;
    swal_alert("Gagal !", "File Harus PDF !", "error");
  }
}
function checkextensionCV4() {
  const file = document.querySelector("#cvAnggota4");
  if (/\.(pdf)$/i.test(file.files[0].name) === false) {
    file.value = null;
    swal_alert("Gagal !", "File Harus PDF !", "error");
  }
...
function check_status_anggota() {
  ...
  $.get(
    `${BASE_URL}C_penelitian_plp_pnbp/check_permintaan_anggota/${
      ...
      if (res.code <= 3) {
        if (res.code == 1) {
          ...
          res.pesan.every((resnya) => {
            console.log(`status permintaan = ${resnya.status_permintaan}`);
...
const element = {
  ...
  identitas_tkt: "input[name='identitas_tkt']",
  ...
  nidn_anggota3: "#anggota3",
  nidn_anggota4: "#anggota4",
  cv_ketua: "#cvKetua",
  cv_anggota1: "#cvAnggota1",
  cv_anggota2: "#cvAnggota2",
  cv_anggota3: "#cvAnggota3",
  cv_anggota4: "#cvAnggota4",
};
...
$(element.jnsproposal).on("change", function () {
  ...
  $.ajax({
    ...
    success: function (val) {
      data = JSON.parse(val);
      // console.log(data);
      $("#biayadiusulkan").val(data[0].biaya_proposal);
      $("#skema-luaran").html(data[1]);      
    },
  });
});
...
$(element.luaran_tambahan).on("change", function () {
  if ($(this).is(":checked")) {
    ...
    $(".tambahan_luaran").html(
      `<input type="text" value="" class="form-control" id="luaran12" placeholder="Luaran 1, Luaran 2, Luaran 3 dst." name="tambahan_luaran">`
    );
  } else {
...
function validasi_identitas_usulan() {
  ...
  const jnsproposal = $(`${element.jnsproposal}:checked`).val();
  // const fokus = $(`${element.fokus_penelitian}:checked`).val();
  const tema = $(`${element.tema_penelitian}:checked`).val();
  ...
  const identitas_tkt = $(element.identitas_tkt).val();
  ...
  if (identitas_tkt === "") {
    state.identitas_tkt = false;
    $(error.identitas_tkt).show();
  } else {
    state.identitas_tkt = true;
    $(error.identitas_tkt).hide();
  }
  ...
  if (metode === "") {
    ...
    if (metode.split(" ").length > 2000) {
      swal_alert(
        "Banyak Kata Tidak Sesuai",
        "Maksimal kata tinjauan metode adalah 2000 kata",
        "error"
      );
      state.metode = false;
    } else {
...
let make_anggota3 = false;
let make_anggota4 = false;
...
const select_dosen = function () {
  ...
  //cari dosen anggota3
  $("#anggota3").change(function () {
    const nidn2 = $(this).val();
    if (nidn2 !== "") {
      const data2 = find_dosen(nidn2);
      $("#dsn_nama3").val(data2.nama);
      $("#dsn_jurusan3").val(data2.unit);
      $("#dsn_pangkat3").val(data2.jenis_job);
      $("#id_sinta4").val(data2.sinta);
    } else {
      $("#dsn_nama3").val("");
      $("#dsn_jurusan3").val("");
      $("#dsn_pangkat3").val("");
  ...
  //cari dosen anggota4
  $("#anggota4").change(function () {
    const nidn3 = $(this).val();
    if (nidn3 !== "") {
      const data3 = find_dosen(nidn3);
      $("#dsn_nama4").val(data3.nama);
      $("#dsn_jurusan4").val(data3.unit);
      $("#dsn_pangkat4").val(data3.jenis_job);
      $("#id_sinta5").val(data3.sinta);
    } else {
      $("#dsn_nama4").val("");
      $("#dsn_jurusan4").val("");
      $("#dsn_pangkat4").val("");
...
function cek_anggota2() {
  $('input[name="pakegak"]').on("change", () => {
    if ($('input[name="pakegak"]').is(":checked")) {
      make_anggota2 = true;
      console.log($('input[name="cv_lama_anggota2"]').length);
...
function cek_anggota3() {
  $('input[name="pakegak3"]').on("change", () => {
    if ($('input[name="pakegak3"]').is(":checked")) {
      make_anggota2 = true;
      console.log($('input[name="cv_lama_anggota3"]').length);
      if ($('input[name="cv_lama_anggota3"]').length !== 1) {
        $("#nextBtn").prop("disabled", true);
        $("#b_test2").show();
      }
    } else {
    ...
    $(".form-anggota3").toggle();
  });
}


function cek_anggota4() {
  $('input[name="pakegak4"]').on("change", () => {
    if ($('input[name="pakegak4"]').is(":checked")) {
      make_anggota2 = true;
      console.log($('input[name="cv_lama_anggota4"]').length);
      if ($('input[name="cv_lama_anggota4"]').length !== 1) {
        $("#nextBtn").prop("disabled", true);
        $("#b_test2").show();
      }
    } else {
...
$("#b_test2").click(function () {
  ...
  const identitas_tkt = $('input[name="identitas_tkt"]').val();
  ...
  form_data.append("identitas_tkt", identitas_tkt);
  ...
  $.ajax({
    ...
    url: `${
      $(this).val() === "simpan"
        ? `${BASE_URL}C_penelitian_plp_pnbp/store_pengajuan_edit_satu`
        : `${BASE_URL}C_penelitian_plp_pnbp/store_pengajuan_edit_satu`
    }`,
    data: form_data,
    contentType: false,
    ...
    success: function (data) {
      if (data.code === 1) {
        ...
        setTimeout(function () {
          window.location.replace(`${BASE_URL}C_penelitian_plp_pnbp`);
        }, 2000);
      } else {
        $("#overlay").fadeOut(400);
...
$("#b_test").click(function () {
  if($("#proposal").val() != '' || $("#rab").val() != '' ) {
    const proposal = $("#proposal")[0];
    const rab = $("#rab")[0];
    let err_msg = "";
    let state_size = true;
    if($("#proposal").val() != ""){
      if(proposal.files[0].size > 3145728){
        err_msg += "File Proposal";
        state_size = false;
      }
    }

    if($("#rab").val() != ""){
      if(rab.files[0].size > 3145728){
        if(!state_size){
          err_msg += " & File RAB";
        }else { 
          err_msg += "File RAB";
    ...
    err_msg += " Tidak Boleh Lebih Dari 3 MB!";
  ...
  $.ajax({
    type: "POST",
    dataType: "json",
    url: `${BASE_URL}C_penelitian_plp_pnbp/store_pengajuan_edit_dua`,
    data: form_data,
    contentType: false,
    cache: false,
    processData: false,
