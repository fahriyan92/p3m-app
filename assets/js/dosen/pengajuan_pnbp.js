...
function check_status_anggota() {
  ...
  $.get(
    `${BASE_URL}C_penelitian_dsn_pnbp/check_permintaan_anggota/${id_pengajuan == undefined ? 0 : id_pengajuan
    }`,
    function (data) {
...
$(element.jnsproposal).on("change", function () {
  var id = $(this).val();
  $.ajax({
    url: `${BASE_URL}C_penelitian_dsn_pnbp/cari_biaya`,
    method: "POST",
    data: { id: id },
...
function validateForm() {
  ...
  if (currentTab === 0) {
    if (validasi_identitas_usulan() == 1) {
      swal_alert(
        "Berhasil",
        "Identitas Usulan Lengkap",
        "success"
      );
      return true;
    }
  ...
  if (currentTab === 1) {
    if (validasi_identitas_dosen() == 1) {
      swal_alert(
        "Berhasil",
        "Menambahakan Anggota Baru",
        "Silahkan menunggu persetujuan anggota, klik simpan terlebih dahulu",
        "success"
      );
      return true;
    };
...
$("#b_test2").click(function () {
  ...
  $.ajax({
    ...
    dataType: "json",
    url: `${$(this).val() === "simpan"
      ? `${BASE_URL}C_penelitian_dsn_pnbp/store_pengajuan`
      : `${BASE_URL}C_penelitian_dsn_pnbp/store_pengajuan`
      }`,
    data: form_data,
    contentType: false,
...
$("#b_test").click(function () {
  if ($("#proposal").val() == "" || $("#rab").val() == "") {
    ...
    if (!state_size) {
      swal_alert("Gagal", err_msg, "error");
  ...
  $.ajax({
    type: "POST",
    dataType: "json",
    url: `${BASE_URL}C_penelitian_dsn_pnbp/store_pengajuan_kedua`,
    data: form_data,
    contentType: false,
    cache: false,
    processData: false,
