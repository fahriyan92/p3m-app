...
class M_mandiri extends CI_Model
{ 
    public function id_pengajuan()
    {
        $this->db->select('id_pengajuan');
        $this->db->where(['nip_ketua' => $this->session->userdata('nidn') ,'id_jenis' => 1]);
    ...
    private function check_ketua_registered($id_detail){
        $data = $this->db->query("select 1 from anggota_dosen_mandiri where nip = '".$this->session->userdata('nidn')."' AND id_pengajuan_detail = ".$id_detail)->row();
        if($data == null){
            return true;
        }
        //kalo true berati dosen tidak ada , dan akan diinsert ke table anggota dosen mandiri (ketuanya. karena diambil berdasarkan session nidn = nip)
        return false;
    }
    ...
    public function insert_identitas($data)
    {
        $this->db->trans_start();
            ...
            if($id_pengajuan !== null){
                ...
                $this->db->insert('pengajuan_mandiri',['status' => 1, 'nip_ketua' => $this->session->userdata('nidn'), 'created_at' => $data['created_at'], 'id_jenis' => 1]);
            ...
            if($this->check_ketua_registered($id_detail)){
                $this->db->insert('anggota_dosen_mandiri',['id_pengajuan_detail' => $id_detail, 'nip' => $this->session->userdata('nidn'), 'status' => 2, 'created_at' => date('Y-m-d H:i:s')]);
            }
    ...
    public function get_all_proposal()
    {
		...
		$this->db->where(['a.status >' => 7, 'c.id_jenis' => 1, 'i.nip' => $this->session->userdata('nidn'), 'a.status_koreksi' => 1]);
	...
	public function get_proposalnya_mandiri($id)
	{
        ...
        $this->db->join('komentar_mandiri j', 'j.id_pengajuan_detail = a.id_pengajuan_detail');
		$this->db->where(['a.id_pengajuan_detail' => $id, 'e.id_jenis' => 1, 'a.status_koreksi' => 1, 'a.status' => 8]);
