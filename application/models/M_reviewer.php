...
class M_reviewer extends CI_Model
{
	...
	function reviewer_get_all()
	{
		$this->db->select("nidn");
		$this->db->where("status", 1);
		$query1 = $this->db->get("tb_reviewer")->result();
		if (!empty($query1)) {
			$listId = "";
	...
	public function list_reviewer_id($id)
	{
		return $this->db->get_where('tb_reviewer', ['id_reviewer' => $id])->row();
	}

	public function list_reviewer_asc()
	{
		$this->db->select('tb_reviewer.id_reviewer, tb_reviewer.id_event,dummy_dosen.nama, tb_reviewer.status');
		$this->db->join('dummy_dosen', 'dummy_dosen.nidn = tb_reviewer.nidn');
		// $this->db->join('tb_event', 'tb_event.id_event = tb_reviewer.id_event');
		$this->db->where('status', '1');
		$this->db->order_by('id_reviewer', 'asc');
		return $this->db->get('tb_reviewer')->result();
	}
	...
	public function cek_nidn($id, $tahun)
	{
		...
		return $this->db->get('tb_reviewer')->num_rows();
	}
	...
	public function anggota_mhs_Byproposal($id)
	{
		...
		$this->db->select('a.nim, a.nama, a.prodi, a.id_pengajuan_detail, a.status, a.id_anggota_mhs');
	...
	public function get_proposalByReviewer_revisi($reviewer, $where, $id_jenis_event)
	{
		$this->db->select('*');
		$this->db->from('tb_kerjaan_reviewer as a');
		$this->db->join('tb_reviewer as rr', 'rr.id_reviewer = a.id_reviewer');
	...
	{
		$this->db->select('a.id_det_pengajuan, a.id_pengajuan_detail, a.judul, a.tahun_usulan, b.id_pengajuan_detail, dd.id_list_event, dd.nidn_ketua, dd.status, c.nidn, c.nip, c.nama, c.pangkat, b.updated_at,b.created_at, klp.nama_kelompok');
		$this->db->from('tb_identitas_pengajuan as a');
		$this->db->join('tb_pengajuan_detail as b', 'b.id_pengajuan_detail = a.id_pengajuan_detail');
		$this->db->join('tb_pengajuan as dd', 'dd.id_pengajuan = b.id_pengajuan');
		$this->db->join('dummy_dosen as c', 'c.nidn = dd.nidn_ketua');
	...
	public function status_kerjaan_revisi($id_reviewer, $di_pengajuan)
	{
		$this->db->select('id_kerjaan,updated_at dinilai, kerjaan_selesai status');
		$this->db->from('tb_kerjaan_reviewer');
		$this->db->where(['id_reviewer' => $id_reviewer, 'id_pengajuan_detail' => $di_pengajuan]);
		return $this->db->get()->row();
	}
	...
	public function get_Jenis_Soal3()
	{
		$this->db->select('*');
		$this->db->from('tb_jenis_soal');
	...
	public function get_MasterReviewer($id_event)
	{
		$this->db->select('*');
		$this->db->from('tb_reviewer as a');
	...
	{
		$query = "
			SELECT nilai_satu.id_kerjaan,dosen.nama,dosen.nip,dosen.unit, IF(ROUND(SUM(nilai_satu.nilai_sementara),2) != 0.00,ROUND(SUM(nilai_satu.nilai_sementara),2), 'Belum Review')  AS nilai_fix FROM
				(SELECT kj.id_kerjaan,kj.id_reviewer,kj.id_pengajuan_detail,
						(
									CASE
											...
											WHEN js.id_event = 3 THEN IF(js.id_jenis_soal = 5,(SUM((pl.score / 7)*js.bobot*pl.prosentase))/100,SUM((pl.score * pl.prosentase))/100)
											ELSE 0
									END) AS nilai_sementara FROM
				tb_kerjaan_reviewer kj
				LEFT JOIN tb_penilaian pn ON kj.id_kerjaan = pn.id_kerjaan
				LEFT JOIN tb_pilihan as pl ON pl.id_pilihan = pn.id_pilihan
				LEFT JOIN tb_soal sl ON sl.id_soal = pl.id_soal AND pl.status = 1
				LEFT JOIN tb_jenis_soal js ON sl.id_jenis_soal = js.id_jenis_soal AND sl.status = 1
				WHERE kj.id_pengajuan_detail = " . $id . " GROUP BY kj.id_kerjaan,js.id_jenis_soal) AS nilai_satu
					INNER JOIN tb_reviewer  ON nilai_satu.id_reviewer = tb_reviewer.id_reviewer
					INNER JOIN dummy_dosen dosen ON tb_reviewer.nidn = dosen.nidn WHERE nilai_satu.id_pengajuan_detail = " . $id . " GROUP BY nilai_satu.id_kerjaan
		";
