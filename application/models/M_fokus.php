...
class M_fokus extends CI_Model
{
    function all_data()
    {
        return $this->db->get('tb_fokus');
    }
    function all_dataskema()
    {
        return $this->db->get('tb_fokus');
    }
    ...
    public function kel_luaran_penelitian_dosen($table)
    {
        $this->db->from($table);
        $this->db->where(["flag_pengabdian is NULL" => null, "flag_plp is NULL" => null, "status" => 1]);
        return $this->db->get();
    }

    public function kel_luaran_pengabdian_dosen($table)
    {
        $this->db->from($table);
        $this->db->where(["flag_pengabdian" => "Y"]);
        $this->db->where(["status" => "1"]);
        return $this->db->get();
    }

    public function kel_luaran_penelitian_plp($table)
    {
        $this->db->from($table);
        $this->db->where(["flag_plp" => "Y"]);
        return $this->db->get();
    }

    public function list_tema($id_event)
    {
