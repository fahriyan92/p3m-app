...
class C_settings_user extends CI_Controller
{
	...
	public function index()
	{
		...
		$data['pendidikans'] = $this->list_pendidikan();
		$data['prodi'] = $this->list_prodi();
		$data['dosen'] = $this->setting->dosen_profile($this->session->userdata('nidn'))->result();

		$this->load->view('index', $data);
	}

	private function list_pendidikan(){
		return $this->db->query("select * from pendidikan_terakhir")->result();
	}
