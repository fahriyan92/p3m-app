...
class Proposal_penelitian extends RestController
{
    ...
    public function index_get()
    {
        ...
        $nip_anggota = $this->get('nip_anggota');
        ...
        $q_pnbp = "
            ...
            c.judul AS judul,
            c.tahun_usulan AS tahun_usulan,
        ...
        $w_pnbp = " WHERE a.id_pengajuan_detail IN (SELECT DISTINCT id_pengajuan_detail FROM tb_kerjaan_reviewer WHERE kerjaan_selesai = 1)  AND d.id_jenis_event = 1 AND a.status_keputusan = 1";
        $w_mandiri = " WHERE b.id_jenis = 1 AND a.status_koreksi = 1 AND a.status = 8";


        $add_thn = " AND c.tahun_usulan ='" . $thn . "' ";
        $add_dsn = " AND b.nip_ketua ='" . $nik_dsn . "' ";
        $add_dsn_pnbp = " AND b.nidn_ketua ='" . $nik_dsn . "' ";
        $add_judul = " AND c.judul ='" . $judul . "' ";

        if ($nip_anggota != null) {
            $q_pnbp .= "INNER JOIN tb_anggota_dosen h ON a.id_pengajuan_detail = h.id_pengajuan_detail";
            $q_mandiri .= "INNER JOIN anggota_dosen_mandiri g ON g.id_pengajuan_detail = a.id_pengajuan_detail";

            $w_pnbp .= " AND h.nidn = '" . $nip_anggota . "'";
            $w_mandiri .= " AND g.nip = '" . $nip_anggota . "'";
        }

        $q_mandiri .= $w_mandiri;
        $q_pnbp .= $w_pnbp;

        //$add_id_proposal = "AND a.id_pengajuan_detail ='".$detail."' ";
        ...
        if ($coba != null) {
            ...
            foreach ($coba as $value) {
                $jadi[$i]["id_proposal"] = $value->id;
                $jadi[$i]["judul"] = $value->judul;
                $jadi[$i]["program"] = $value->program;
                $jadi[$i]["tahun_pelaksanaan"] = $value->tahun_usulan;
                $jadi[$i]["tanggal_awal"] = $value->tanggal_awal;
                $jadi[$i]["tanggal_sampai"] = $value->tanggal_sampai;
                $jadi[$i]["file_proposal"] = "https://layanan-p3m.polije.ac.id/assets/berkas/file_proposal/" . $value->file_proposal;
                ...
                $q = "SELECT b.nip AS nip, b.nidn_real AS nidn, b.nama AS nama FROM " . $tabel[$value->program] . " a INNER JOIN dummy_dosen b ON a." . $field[$value->program] . " = b.nip WHERE " . $alias[$value->program] . "." . $field[$value->program] . " != '" . $value->nip_ketua . "' AND a.id_pengajuan_detail = " . $value->id;
