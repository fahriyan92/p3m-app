...
class C_master_data extends CI_Controller
{
    ...
    public function update_profile()
    {
        ...
        $link_scholar = $post['link_google_scholar'];
        $link_scopus = $post['link_scopus'];
        ...
        if ($email !== '') {
            ...
            $data = [
                ...
                'link_googlescholar' => $link_scholar,
                'link_scopus' => $link_scopus
            ];
            $this->db->trans_start();
            ...
            $limit_penelitian = 1;
            if($this->session->userdata('job') == 'dosen')
            {
                $limit_penelitian = 2;
            }
            if ($av_hindex != null) {
                ...
                $this->db->insert('tb_hindex', ['nama_dosen' => $nama->nama, 'nidsn_dosen' => $this->session->userdata('nidn'), 'h_index_scopus' => $hindex_scopus, 'h_index_schoolar' => $hindex, 'limit_penelitian' => $limit_penelitian, 'sisa_limit_penelitian' => $limit_penelitian]);
            }
            $this->db->where('nip', $nip);
    ...
    public function jenis_proposal()
    {
        ...
        $data['brdcrmb'] = 'Beranda / Master Data / Data Skema Penelitian';
    ...
    //tambahan niko buat skema pengabdian
    public function jenis_proposal_pengabdian()
    {
        $data['content'] = VIEW_ADMIN . 'content_jns_proposal_pengabdian';
        $data['judul'] = 'Skema Pengabdian';
        $data['brdcrmb'] = 'Beranda / Master Data / Data Skema Pengabdian';
        $kelompok = $this->db->query("select * from tb_kelompok_pengajuan where flag_pengabdian = 'Y'")->result();
        // $data['kelompok'] = $this->Mfokus->get_all('tb_kelompok_pengajuan')->result();
    ...
    public function store_jenis_proposal_pengabdian()
    {
        $post = $this->input->post();
        ...
        $this->insert_log(['CREATE', 'CREATE PADA SKEMA(JENIS PROPOSAL PENGABDIAN)']);
        echo json_encode($this->response([1, 'Berhasil Menyimpan Data']));
    }
    ...
    public function store_jenis_proposal()
    {
        ...
        $this->insert_log(['CREATE', 'CREATE PADA SKEMA(JENIS PROPOSAL)']);
        echo json_encode($this->response([1, 'Berhasil Menyimpan Data']));
    }
    ...
    public function update_jnsproposal()
    {
        ...
        $this->insert_log(['UPDATE', 'UPDATE PADA SKEMA(JENIS PROPOSAL)']);
        print_r($this->response([1, 'berhasil Mengedit Data']));
        return;
    }
    ...
    {
        $data['content'] = VIEW_ADMIN . 'content_master_pemonev';
        $data['judul'] = 'Master Pemonev';
        $data['brdcrmb'] = 'Beranda / Master Data / Data Master Pemonev';
        $dataDosen = $this->pemonev->pemonev_get_all();
        if ($dataDosen == null) {
            $data['dosen'] = $this->pemonev->get_all("dummy_dosen")->result();
        } else {
            $data['dosen'] = $dataDosen;
        }
        $data['dosen2'] = $this->pemonev->get_all("dummy_dosen")->result();
        $data['event'] = $this->event->get_dataEvent()->result();

        $this->load->view('index', $data);
    }

    public function get_list_master_pemonev()
    {
        $list = $this->pemonev->list_pemonev_asc();
        $a = 0;
        $pemonev = "";
        for ($i = 0; $i <= count($list) - 1; $i++) {
            $eventGroup = "";
            $a++;
            $pemonev .= '<tr>';
            $pemonev .= '<td class="text-uppercase">' . $a . '.</td>';
            $pemonev .= '<td class="text-uppercase">' . $list[$i]->nama . '</td>';
            $event = explode(",", $list[$i]->id_event);
            for ($j = 0; $j < count($event); $j++) {
                $listEvent = $this->event->list_event_id2($event[$j]);
                $eventGroup .= $listEvent[0]->nm_event . ", ";
            }
            $pemonev .= '<td class="text-uppercase">' . substr($eventGroup, 0, -2) . '</td>';
            $pemonev .= '<td class="text-center"><a href="#" id="' . $list[$i]->id_pemonev . '" class="btn btn-xs btn-warning btn-edit"><i class="fa fa-fw fa-edit"></i></a><a href="#" id="' . $list[$i]->id_pemonev . '" class="btn btn-xs btn-danger btn_delete"><i class="fa fa-fw fa-trash"></i></a></td>';
            $pemonev .= '</tr>';
        }

        echo json_encode($pemonev);
    }

    public function store_MasterPemonev()
    {
        $post = $this->input->post();
        ...
        if ($post['nidn'] === '' || $eventIds === '') {
            ...
            $cek_nidn = $this->pemonev->cek_nidn($post['nidn'], date("Y"));
            if ($cek_nidn == 0) {
                $soal = [
                    'nidn' => $post['nidn'],
                    'id_event' => $event,
                ...
                $insert = $this->pemonev->insert("tb_pemonev", $soal);
            } else {
                echo json_encode($this->response([0, 'Dosen Sudah Didata Ditahun Ini!']));
                return;
            }
    ...
    public function get_list_pemonev_by_id()
    {
        $id = $this->input->post('id');

        $get_list = $this->pemonev->list_pemonev_id($id);
        echo json_encode($get_list);
    }

    public function updatePemonev()
    {
        $id = $this->input->post('id_pemonev');
        $pemonev = $this->input->post('pemonev');
        $eventIds = $this->input->post('event');
        $event = implode(",", $eventIds);
        $status = $this->input->post('status');

        if ($pemonev === '' || $event === '' || $status === '') {
            echo json_encode($this->response([0, 'Form Hurus Lengkap!']));
            return;
        } else {
            ...
            $this->pemonev->update('tb_pemonev', $data, array('id_pemonev' => $id));
            echo json_encode($this->response([1, 'Berhasil Mengubah Data']));
        }
    }


    public function deletePemonev()
    {
        $post = $this->input->post();
        $id_list = $post['id'];
        ...
        $this->db->where('id_pemonev', $id_list);
        $delete = $this->db->update('tb_pemonev', $data);
        // return ($this->db->affected_rows() > 0);
    ...
    public function Pemonev1()
    {
        $this->load->model('M_Pemonev1');
        $data['Pemonev'] = $this->M_Pemonev1->getPemonev();

        $this->load->view('content/admin/V_Pemonev', $data);
    }
