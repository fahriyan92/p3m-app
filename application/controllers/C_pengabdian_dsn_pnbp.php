...
class C_pengabdian_dsn_pnbp extends CI_Controller
{
    ...
    public function cari_biaya()
    {
        ...
        $get_luaran = $this->Mfokus->kel_luaran_pengabdian_dosen('tb_luaran')->result();
        $myArray = explode(',', $get_biaya->kelompok_luaran);
        // echo json_encode($get_luaran);
        // return;
        $html .= '<label for="target">A. Wajib11</label><br>';
    ...
    {
        $sql = "select waktu_mulai mulai,waktu_akhir selesai from tb_list_event where id_jenis_event= 2 AND id_tahapan = 2 AND YEAR(waktu_mulai) = YEAR(CURDATE()) AND status = 1";
    ...
    {
        $sql = "select id_list_event id_event, waktu_mulai mulai,waktu_akhir selesai from tb_list_event where id_jenis_event = 2 AND id_tahapan = 2 AND (curdate() between waktu_mulai AND waktu_akhir) AND status = 1";
        $data = $this->db->query($sql)->row();
    ...
    {
        $sql = "select id_list_event id_event, waktu_mulai mulai,waktu_akhir selesai from tb_list_event where id_jenis_event = 2 AND id_tahapan = 11 AND (curdate() between waktu_mulai AND waktu_akhir) AND status = 1;";
        $data = $this->db->query($sql)->row();
    ...
    public function halaman_identitas()
    {
        ...
        $data['fokus'] = $this->Mfokus->get_fokus(2);
    ...
    public function preview($id = null)
    {
        if ($id == null) {
            ...
            redirect("C_pengabdian_dsn_pnbp");
        }
        ...
        // content
        $username = $this->session->userdata('username');
        $data['content'] = VIEW_DOSEN . 'content_preview_pengabdian_dsn';
        $data['username'] = $username;
        ...
        $data['pengajuan_evaluasi_url'] = 'C_pengabdian_dsn_pnbp/halaman_pengajuan_evaluasi/' . $id . '';
        $data['edit_pengajuan_evaluasi_url'] = 'C_pengabdian_dsn_pnbp/halaman_edit_evaluasi/' . $id . '';
        $data['cek_edit'] = $count;
...
//         return;
    ...
    public function halaman_pengajuan_evaluasi($id = null)
    {
        $cek_event = $this->get_id_event_evaluasi();

        // content
        $cek = $this->get_id_event_evaluasi();
        $username = $this->session->userdata('username');
        $data['username'] = $username;
        $data['judul'] = 'Pengajuan Evaluasi Pengabdian PNBP';
        $data['brdcrmb'] = 'Beranda / Pengabdian Dosen / PNBP / Evaluasi';
        $data['idproposal'] = $id;
        $kelompok_pengajuan = $this->pemonev->get_kelompok_pengajuan($id);
        $data['namakelompok'] = $kelompok_pengajuan->nama_kelompok;
        $data['dt_proposal'] = $this->reviewer->get_proposalnya($id);
        $data['berkas'] = $this->pengabdian->dataBerkas_evaluasi($id);
        $data['deskripsi'] = $this->pengabdian->deskripsiBerkas_evaluasi($id);
        $berkas = $data['dt_proposal'];
        ...
        $data['content'] = VIEW_DOSEN . '/evaluasi/pengabdian/content_pengabdian_evaluasi_dosen_pnbp';
//         var_dump($berkas->id_pengajuan_detail);
        $this->load->view('index', $data);
    }
    ...
    private function namabaru_evaluasi($n, $p, $s)
    {
        ...
        $skema = trim($s);
        $skema_fix = "pengabdian";
        $nama_file = $nama_fix . "_" . $nip . "_" . $skema . "_" . time();
        return strtolower($nama_file);
    }
    ...
    {
		$sql = "select nidn,nama,jenis_job,jenis_unit,unit,sinta,scopus,file_cv from dummy_dosen, tb_hindex b WHERE file_cv != '' AND email != ''  AND jenis_job = 'dosen' AND nidn = b.nidsn_dosen AND b.limit_pengabdian > 0";
        $query = $this->db->query($sql)->result();
    ...
    public function halaman_edit($id = null)
    {
        ...
        if ($cek_event === null) {
            $this->session->set_flashdata('error_access', 'Event Saat ini Sudah Di tutup');
            redirect("C_pengabdian_dsn_pnbp/preview/$id");
        } else {
            $id_eventnya = $cek_event->id_event;
        }
    ...
    {
        $dir_cv = realpath(APPPATH . '../assets/berkas/file_monevs');
        $name = $_FILES['file_laporan']['name'];
    ...
    public function simpan_progresspengabdian($id)
    {
        $detail = $this->reviewer->get_proposalnya($id);
        $id_proposal = $detail->id_pengajuan_detail;
        ...
        $data = array(
            'id_pengajuan_detail' => $id_proposal,
            'judul_luaran' => $targetLuaran,
        ...
        $this->reviewer->insert('tb_progresspengabdian', $data);

        $res = $this->response([1, 'Berhasil Tambah ']);
        echo json_encode($res);

        $this->session->set_flashdata('success', 'Berhasil Simpan Review');
        redirect('C_pengabdian_dsn_pnbp/preview/' . $id_proposal);
