...
class C_rekap extends CI_Controller
{
	...
	public function list_proposal_pnbp_pemonev()
	{
		$tahun = $this->input->post('tahun');
		$jenis = $this->input->post('jenis');
		$status = $this->input->post('status');
		...
		$pengajuan = $this->pemonev->get_proposal_list($tahun, $jenis, $status == 0 ? NULL : $status);
		echo json_encode(['status' => 'ok', 'data' => $pengajuan]);
		return;
	}
	...
	public function list_pemonev_score()
	{
		$id_pengajuan_detail = $this->input->post('id');
		$data = $this->pemonev->get_pemonev_score($id_pengajuan_detail);
		echo json_encode($data);
		return;
	}
	...
	public function viewDetail($id = '')
	{
		...
		$data['berkas_eval_penelitian'] = $this->pemonev->dataBerkas_evaluasi_penelitian($id);
		$data['berkas_eval_pengabdian'] = $this->pemonev->dataBerkas_evaluasi_pengabdian($id);
