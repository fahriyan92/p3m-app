...
class Anggota_dosen_revisi extends CI_Controller{
    ...
    public function index()
    {
        ...
        if(!$cek){
            ...
            redirect('C_penelitian_dsn_mandiri/mandiri_pengajuan_pengabdian');
        }
        $view = VIEW_DOSEN . 'pengajuan_mandiri_pengabdian/anggota_dosen_revisi.php';
    ...
    private function list_dosen($id)    
    {
        ...
        $sql = "select nidn,nama,jenis_job,jenis_unit,unit,file_cv,sinta,scopus from dummy_dosen where jenis_job='dosen' AND nidn NOT IN((SELECT nip FROM anggota_dosen_mandiri WHERE id_pengajuan_detail = $id AND nip != '$nip_ketua' ))";
    ...
    public function get_anggota_dosen($id = null)
    {
        ...
        $sql = "SELECT id_pengajuan_detail, id_anggota,a.sinta, a.nip, b.file_cv,a.status,b.nama, b.unit,b.jenis_unit FROM anggota_dosen_mandiri a INNER JOIN dummy_dosen b ON a.nip = b.nidn WHERE id_pengajuan_detail = '$id_pengajuan' AND a.status != 0 ";
        if($id !== null){
    ...
    public function store()
    {
        ...
        $kirim = ['id_pengajuan_detail' => $id, 'nip' => $nip, 'status' => $status, 'created_at' => date('Y-m-d H:i:s')];
