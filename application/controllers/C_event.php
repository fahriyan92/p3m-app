...
class C_event extends CI_Controller
{
    ...
    public function check_deletable($id = null){
        ...
        $query = "
            ...
            select id from view_list_event where tahapan = 'penerimaan proposal' AND status = 1  AND date_format(mulai,'%Y') = 
                (
                    select date_format(mulai,'%Y') as tahun from view_list_event where id = ".$id." limit 1
                ) AND id_event = 
                (
                    select id_event from view_list_event where id = ".$id." limit 1
                ) limit 1
            )
            limit 1;
        ";
    ...
    public function get_list_event($tahun = null)
    {
        ...
        for ($i = 0; $i <= count($list) - 1; $i++) {
            if($list[$i]->id_event == 1){
                ...
                $penelitian .= '</tr>';
            } elseif ($list[$i]->id_event == 3) {
                $c++;
                $akhir = $list[$i]->akhir === null ? '-' : tanggal_indo($list[$i]->akhir);
                $penelitian_plp .= '<tr>';
                $penelitian_plp .= '<td class="text-uppercase">' . $c . '.</td>';
                $penelitian_plp .= '<td class="text-uppercase">' . $list[$i]->tahapan . '</td>';
                $penelitian_plp .= '<td class="text-center">' . tanggal_indo($list[$i]->mulai) . '</td>';
                $penelitian_plp .= '<td class="text-center">' . $akhir . '</td>';
                $penelitian_plp .= '<td class="text-center"><a href="#" id="' . $list[$i]->id . '" class="btn btn-xs btn-warning btn-edit"><i class="fa fa-fw fa-edit"></i></a><a href="#" id="' . $list[$i]->id . '" class="btn btn-xs btn-danger btn-delete"><i class="fa fa-fw fa-trash"></i></a></td>';
                $penelitian_plp .= '</tr>';
            } else{ 
                $b++;
                $akhir = $list[$i]->akhir === null ? '-' : tanggal_indo($list[$i]->akhir);
        ...
        echo json_encode([$penelitian,$penelitian_plp,$pengabdian]);
    ...
    public function action_store()
    {
        ...
        //ini validasi agar tanggal tidak tempul (1 tanggal 1 tahapan)
        // if ($tgl_sama !== null) {
