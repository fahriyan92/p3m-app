...
class M_pengabdian extends CI_Model
{
	...
	function dataBerkas_evaluasi($id)
	{
		$this->db->select('*');
		$this->db->from('tb_berkas_evaluasi_pengabdian');
		$this->db->where('id_pengajuan_detail', $id);
		return $this->db->get()->row();
	}
    
    function deskripsiBerkas_evaluasi($id)
	{
		$this->db->select('*');
		$this->db->from('tb_deskripsi_evaluasi_pengabdian');
		$this->db->where('id_pengajuan_detail', $id);
		return $this->db->get()->row();
	}
	...
	public function up_berkas_wajib_1($id)
	{
		$sql = "UPDATE `tb_berkas_evaluasi_pengabdian` SET `w_b2` = 'kontol' WHERE `tb_berkas_evaluasi_pengabdian`.`id_pengajuan_detail` = $id;";
