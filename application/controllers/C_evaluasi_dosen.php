...
class C_evaluasi_dosen extends CI_Controller
{
    ...
    public function index()
    {
        ...
        if ($this->session->userdata('level') == "4") {
            ...
            $data['content'] = VIEW_SUPERADMIN . 'content_menu_evaluasi';
        } elseif ($this->session->userdata('level') == "1") {
            $data['role'] = 'Admin';
            $data['content'] = VIEW_ADMIN . 'content_menu_evaluasi';
            ...
            $data['content'] = VIEW_DOSEN . 'content_menu_evaluasi';
        } elseif ($this->session->userdata('level') == "3") {
            $data['role'] = 'Reviewer';
            $data['mandiri'] = $this->list_judul_mandiri();
            $data['content'] = VIEW_REVIEWER . 'content_menu_evaluasi';
    ...
    public function tombol_permintaan()
    {
        ...
        if ($id != '') {
            ...
            if ($status == 2) {
                ...
                $cek = $this->dosen->check_tipe_pengajuan($input['id_proposal'])->row();
        ...
        if (!empty($anggota)) {
            ...
            if ($notif->num_rows() > 0) {
                ...
                foreach ($notif->result() as $key) {
                    $judulnya =  substr(strip_tags(strtolower($key->judul)), 0, 60);
                    if (strlen($key->judul) <= 60) {
                        $judulnya = strip_tags(ucfirst(strtolower($key->judul)));
                    } else {
                        $judulnya = strip_tags(ucfirst(strtolower($key->judul)));
                    }
                    // $judulnya =  substr(strip_tags(ucfirst(strtolower($key->judul))), 0, 60) ;
                    $output = $output . '
					       <tr>
					         <td>' . $no . '</td>
					         <td>' . $judulnya . '</td>
					         <td>
					           <a href="' . base_url() . 'C_monitoring/detail_proposal/' . $key->id_pengajuan_detail . '" class="btn btn-xs btn-danger"> <i class="fa fa-paste"></i></a>
					         </td>
					       </tr>';
