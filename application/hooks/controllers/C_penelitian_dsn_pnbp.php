...
class C_penelitian_dsn_pnbp extends CI_Controller
{
    ...
    {
        $sql = "select waktu_mulai mulai,waktu_akhir selesai from tb_list_event where id_jenis_event= 1 AND id_tahapan = 2 AND YEAR(waktu_mulai) = YEAR(CURDATE())";
    ...
    public function get_id_event()
    {
        ...
        if ($tgl) {
            $tgl_now = date('Y-m-d');
            $sql = "select id_list_event id_event from tb_list_event where  id_jenis_event= 1 AND id_tahapan = 2 AND '" . $tgl_now . "' between '" . $tgl->mulai . "' and '" . $tgl->selesai . "'";
            $data = $this->db->query($sql)->row();
        } else {
            $data = null;
        }
    ...
    {
        $sql = "select a.*,b.status_permintaan status_notifikasi from tb_anggota_dosen a join tb_permintaan_anggota b on a.id_anggota_dsn = b.id_anggota where a.id_pengajuan_detail = " . $id_pengajuan;
        $ketua = [];
        $anggota = [];
