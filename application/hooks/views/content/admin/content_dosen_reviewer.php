    ...
    <section class="content">
        <div class="container-fluid">
            ...
            <div class="card-header d-flex p-0">
              <h3 class="card-title p-3">List Dosen Reviewer <span class="titelnya"></span></h3>
...
<script>
    const get_data = function(){
        ...
        return new Promise(function(resolve,reject){
            $.ajax({
                url: `${BASE_URL}c_reviewer/list_dosen_reviewer/${tahun}`,
                type: "get",
                dataType: 'json',
        ...
        get_data().then(function(res){
            ...
            res.forEach(item => {
                ...
                <tr>
                    <td>${item.nidn}</td>
                    <td>${item.nama}</td>
                    ${item.total != '0' ? `<td>${item.total}</td><td><a class='btn btn-sm btn-primary' href='<?= base_url('c_reviewer/proposalreviewer/') ?>${item.id_reviewer}'><i class="fas fa-fw fa-eye"></i></a></td>` : `<td colspan="2">Data Kosong</td>` }
                </tr>
                `;
                no++;
            });
