                                ...
                                <table id="tabelevent" class="table table-bordered table-striped">
                                    <thead>
                                        <tr>
                                            ...
                                            <th width="15%">Proposal Dimonev</th>
...
<script>
    const get_data = function(){
        ...
        return new Promise(function(resolve,reject){
            $.ajax({
                url: `${BASE_URL}c_reviewer/list_dosen_pemonev/${tahun}`,
                type: "get",
                dataType: 'json',
        ...
        get_data().then(function(res){
            ...
            res.forEach(item => {
                ...
                <tr>
                    <td>${item.nidn}</td>
                    <td>${item.nama}</td>
                    ${item.total != '0' ? `<td>${item.total}</td><td><a class='btn btn-sm btn-primary' href='<?= base_url('c_reviewer/proposalPemonev/') ?>${item.id_pemonev}'><i class="fas fa-fw fa-eye"></i></a></td>` : `<td colspan="2">Data Kosong</td>` }
                </tr>
                `;
                no++;
            });
